...
const AddFlight = () => {
    ...
    const submit = async (e) => {
        ...
        if (Duration === '' && ToPass && FromPass && FlightNoPass && TimePass && Date_Pass) {
            DurationPass = false;
            window.alert('Please Enter Flight Duration')
        }
        else{
        ...
        if (FlightNoPass && FromPass && ToPass && Date_Pass && TimePass && FarePass && DurationPass) {
            setState({
                FlightNo: '', From: '', To: '', Date_: '', TakeOff_Time: '',Duration:'' , Fare: ''
            })

            console.log(FlightNo, From, To, Date_, TakeOff_Time, Fare)
